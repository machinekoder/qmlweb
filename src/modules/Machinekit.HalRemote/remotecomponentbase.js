...
class RemoteComponentBase {
    constructor(debugname, debuglevel) {
        ...
        // Halrcmd
        this.halrcmdChannel = new RPCClient(debugname + " halrcmd", debuglevel);
        this.halrcmdChannel.bind("stateChanged", (state) => {this.halrcmdChannelStateChanged(state);});
    this.halrcmdChannel.bind("socketMessageReceived", (msg) => {this._halrcmdMessageReceived(msg);});
        // more efficient to reuse protobuf messages
        this.halrcmdRx = new Container();
        this.halrcmdTx = new Container();

        // Halrcomp
        this.halrcompChannel = new HalrcompSubscribe(debugname + " halrcomp", debuglevel);
        this.halrcompChannel.bind("stateChanged", (state) => {this.halrcompChannelStateChanged(state);});
    this.halrcompChannel.bind("socketMessageReceived", (msg) => {this._halrcompMessageReceived(msg);});
        // more efficient to reuse protobuf messages
        this.halrcompRx = new Container();

        // callbacks
        this.bind("halrcmdMessageReceived", (msg) => {});
        this.bind("halrcompMessageReceived", (msg) => {});
        this.bind("stateChanged", (state) => {});
    ...
    onFsmHalrcompBindMsgSent(event, from, to) {
        if (this.debuglevel > 0) {
            console.log(this.debugname + ": event HALRCOMP BIND MSG SENT");
        }
    }

    onFsmNoBind(event, from, to) {
        if (this.debuglevel > 0) {
    ...
    onFsmBindConfirmed(event, from, to) {
        if (this.debuglevel > 0) {
    ...
    onFsmBindRejected(event, from, to) {
        if (this.debuglevel > 0) {
    ...
    onFsmHalrcmdTrying(event, from, to) {
        if (this.debuglevel > 0) {
            console.log(this.debugname + ": event HALRCMD TRYING");
        }
    }

    onFsmSyncing(event, from, to) {
        if (this.debuglevel > 0) {
    ...
    onFsmHalrcompUp(event, from, to) {
        if (this.debuglevel > 0) {
            console.log(this.debugname + ": event HALRCOMP UP");
        }
    }

    onFsmSyncFailed(event, from, to) {
        if (this.debuglevel > 0) {
    ...
    onFsmHalrcompTrying(event, from, to) {
        if (this.debuglevel > 0) {
    ...
    onFsmSetRejected(event, from, to) {
        if (this.debuglevel > 0) {
    ...
    onFsmHalrcompSetMsgSent(event, from, to) {
        if (this.debuglevel > 0) {
            console.log(this.debugname + ": event HALRCOMP SET MSG SENT");
        }
    }

    onFsmSyncedEntry(event, from, to) {
        if (this.debuglevel > 0) {
    ...
    bindComponent() {
        console.log("WARNING: slot bind component unimplemented");
    }

    addPins() {
        console.log("WARNING: slot add pins unimplemented");
    }

    removePins() {
        console.log("WARNING: slot remove pins unimplemented");
    }

    unsyncPins() {
        console.log("WARNING: slot unsync pins unimplemented");
    }

    setConnected() {
        console.log("WARNING: slot set connected unimplemented");
    }

    setError() {
        console.log("WARNING: slot set error unimplemented");
    }

    setDisconnected() {
        console.log("WARNING: slot set disconnected unimplemented");
    }

    setConnecting() {
        console.log("WARNING: slot set connecting unimplemented");
    }

    setTimeout() {
        console.log("WARNING: slot set timeout unimplemented");
    }
