...
class RPCClient {
    constructor(debugname, debuglevel) {
        ...
        this.socketRx = new Container();
        this.socketTx = new Container();
        ...
        // callbacks
        this.bind("socketMessageReceived", (msg) => {});
    ...
    onFsmAnyMsgReceived(event, from, to) {
        if (this.debuglevel > 0) {
    ...
    onFsmHeartbeatTimeout(event, from, to) {
        if (this.debuglevel > 0) {
    ...
    onFsmHeartbeatTick(event, from, to) {
        if (this.debuglevel > 0) {
    ...
    resetHeartbeatTimer() {
        ...
        if (this.heartbeatInterval > 0) {
            this.heartbeatTimer = setTimeout(() => { this.heartbeatTimerTick(); }, this.heartbeatInterval);
        }
    ...
    sendSocketMessage(type, tx) {
        ...
        var sendBuffer = encoded.toArrayBuffer();
